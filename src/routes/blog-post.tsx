...
import {
  ...
  getBlogPosts,
  getDocBySlug,
} from "@/features/doc/data/documents.server"
...
export async function loader({ params }: { params: { slug: string } }) {
  ...
  return { slug, doc, toc, previous, next }
}

export function meta({ data }: { data: { doc: Doc } | undefined }) {
  if (!data || !data.doc) {
...
export default function BlogPostPage() {
  const { slug, doc, toc, previous, next } = useLoaderData<typeof loader>()

  // Lazily load the compiled MDX component dynamically using Vite
  const MDXContent = lazy(
    () => import(`../features/doc/content/blog/${slug}.mdx`)
  )
        ...
        <DocGrid>
          ...
          <DocContentCol>
            <Prose className="px-(--page-padding) pt-8 [--page-padding:--spacing(4)]">
              ...
              <TOCInline className="lg:hidden" items={toc as any} />
